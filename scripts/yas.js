...
async function loadKartCounts() {
    ...
    try {
        ...
        return { kartsOnTrackCountYas: 45, kartsInPitCountYas: 10 };
    }
}
...
async function resetColors() {
    if (confirm('Are you sure you want to reset all colors and change the number of karts?')) {
        try {
            const kartsOnTrackCountYas = parseInt(prompt("Enter the number of Karts-on-Track:", "35"), 10);
            const kartsInPitCountYas = parseInt(prompt("Enter the number of Karts-in-Pit:", "10"), 10);

            const validKartsOnTrackCountYas = isNaN(kartsOnTrackCountYas) ? 35 : kartsOnTrackCountYas;
